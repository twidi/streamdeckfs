...
class KeyEvent(KeyFile):
    ...
    def start_repeater(self):
        ...
        self.repeat_thread = Repeater(
            ...
            name=f"{self.kind.capitalize()[:4]}Rep{self.page.number}.{self.key.row}{self.key.col}",
        )
    ...
    def start_waiter(self, duration=None):
        ...
        self.wait_thread = Delayer(
            ...
            name=f"{self.kind.capitalize()[:4]}Wait{self.page.number}.{self.key.row}{self.key.col}",
        )
    ...
    def wait_run_and_repeat(self, on_press=False):
        if self.duration_max:
            self.duration_thread = Delayer(
                ...
                name=f"{self.kind.capitalize()[:4]}Max{self.page.number}.{self.key.row}{self.key.col}",
            )
            ...
            self.duration_thread = Delayer(
                ...
                name=f"{self.kind.capitalize()[:4]}Min{self.page.number}.{self.key.row}{self.key.col}",
            )
            self.duration_thread.start()
        elif self.wait:
            self.start_waiter()
